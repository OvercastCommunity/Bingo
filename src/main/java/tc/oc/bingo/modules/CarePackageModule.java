...
public class CarePackageModule extends BingoModule implements PGMUtils {
  ...
  private final Supplier<Boolean> STREAK_REPEAT = useConfig("streak-repeat", true);
  ...
  public void onChestOpen(PlayerInteractEvent event) {
    ...
    matchPlayer.getMatch().callEvent(new CarePackageOpenEvent(event.getPlayer(), carePackage));
    ...
    ItemStack loot = calculateLoot(matchPlayer);
  ...
  private ItemStack calculateLoot(MatchPlayer player) {
    if (!lootFunctions.isEmpty()) {
      return lootFunctions.get((int) (Math.random() * lootFunctions.size())).apply(player);
    }
  ...
  @Getter
  public class CarePackageOpenEvent extends Event {

    private static final HandlerList handlers = new HandlerList();
    private final Player player;
    private final CarePackage carePackage;

    public CarePackageOpenEvent(Player player, CarePackage carePackage) {
      this.player = player;
