...
public class KillJumpScareObjective extends ObjectiveTracker {
  ...
  // A random chance to strike lightning, put a pumpkin on the player's head, and to play a sound on
  // kill, only is rolled if the player does not currently have a helmet equipped
  ...
  public void onMatchPlayerDeath(MatchPlayerDeathEvent event) {
    ...
    if (killer == null
        || killer.getInventory() == null
        || killer.getInventory().getHelmet() != null) return;
